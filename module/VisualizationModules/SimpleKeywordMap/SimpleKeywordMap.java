...
public final class SimpleKeywordMap extends VisualizationModule implements Runnable{
	...
	private final class RelationPanel extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener{
		...
		public void mousePressed(MouseEvent e){
			...
			if(e.getModifiers()==InputEvent.BUTTON1_MASK){
				...
				int positiveflg=0;
				for(int i=0;i<data.rsuu;i++){
					if(data.balance[i]>0) positiveflg++;
				}
				if(positiveflg==1){
					for(int i=0;i<data.rsuu;i++){
						data.balance[i]=1;
	...
	private void springModel(){
		for(int i=0;i<data.s;i++){
			...
			if(from.k>1) from.E+=to.k*E;
			if(to.k>1) to.E+=from.k*E;
	...
	private synchronized void update(){
		for(int i=0;i<data.k;i++){
			...
			if(kw.set|| kw.set2 || (kw.concent>0) || kw.sindoucountX<-20 || kw.sindoucountY<-20 || kw.mouseset){
				...
				if(kw.sindouX){
					if(kw.dx<0){
						...
						kw.sindoucountX++;
						if(kw.sindoucountX>0) kw.sindoucountX=0;
					}
				}
				if(kw.dx>0) kw.sindouX=true;
				if(kw.sindouY){
					if(kw.dy<0){
						if(Math.abs(kw.dx)>2){
							kw.sindoucountY--;
						...
						kw.sindoucountY++;
						if(kw.sindoucountY>0) kw.sindoucountY=0;
					}
				}
				if(kw.dy>0) kw.sindouY=true;
			}
			...
			kw.dy=0;
			if(kw.x<10) kw.x=10;
			else if(sizex!=0 && kw.x>sizex-10) kw.x=sizex-10;
			if(kw.y<10) kw.y=10;
			else if(sizey!=0 && kw.y>sizey-10) kw.y=sizey-10;
		}
	}
	...
	private final class KeywordPanel extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
		...
		public void paint(Graphics g) {
			...
			for (int i=0;i<data.s;i++){
				...
				from.stopweak=false;
				to.stopweak=false;
				if (from.del || to.del || from.dummy || to.dummy) continue;
				String lbl;
				if(from.focus || from.focus2 || from.focus3 || from.focus4 || from.focus5 || from.focus6 || from.focus7) lbl=from.lbl.substring(0,from.lbl.length()-1);
				else lbl=from.lbl;
				if(to.focus || to.focus2 || to.focus3 || to.focus4 || to.focus5 || to.focus6 || to.focus7) lbl=to.lbl.substring(0,to.lbl.length()-1);
				else lbl=to.lbl;
				int sw=fm.stringWidth(lbl)/2+5;
				int sh=fm.getHeight()/2+2;
				if(rij<data.borderR) continue;
			...
			if(mousesetflg){
					...
					|| (to.x-sw)<mousex && mousex<(to.x+sw) && (to.y-sh)<mousey && mousey<(to.y+sh)){
						...
						if(((from.weaksearchflg && !to.weaksearchflg) || (to.weaksearchflg && !from.weaksearchflg))){
							...
							for(int j=0;j<data.rsuu;j++){
								...
								if(data.spring[i].r[j]>0 && rij<data.borderR) overbordercount++;
							}
							...
							if(colorlink!=null && rhighcount>=1 && overbordercount!=data.rsuu) g.drawString(rlabel,(int)(p[0][0]+(p[1][0]-p[0][0])/(2)),(int)(p[0][1]+(p[1][1]-p[0][1])/(2)));
						}
					}
				...
				for (int i=0;i<data.s;i++){
					...
					to.link1seed=false;
					if (rij<data.borderR|| from.del || to.del || from.dummy || to.dummy) continue;
					int p[][]=new int[2][2];
					p[0][0]=(int)from.x;
					p[0][1]=(int)from.y;
					p[1][0]=(int)to.x;
					...
					int sw=fm.stringWidth(lbl)/2+5;
					int sh=fm.getHeight()/2+2;
					if(from.set || to.set || from.set2 || to.set2) drawSpring(g,rij,p);
				}
			}
			drawKeyword(g);
			drawInfo(g);
		...
		private void drawKeyword(Graphics g){		
			for(int i=0;i<data.k;i++){
				Keyword kw=data.keyword[i];
				if(kw.dummy) continue;
				int x=(int)kw.x;
				int y=(int)kw.y;
				String lbl;
				if(kw.focus || kw.focus2 || kw.focus3 || kw.focus4 || kw.focus5 || kw.focus6 || kw.focus7) lbl=kw.lbl.substring(0,kw.lbl.length()-1);
				else lbl=kw.lbl;
...
//				String lbl=kw.focus2 ? kw.lbl.substring(0,kw.lbl.length()-1) : kw.lbl; //Cut #
				...
				//	else if(kw.focus) color=Color.blue; //@ Blue
					else if(kw.focus2) color=Color.green; //# Light green
				//	else if(kw.focus3) color=new Color(255,0,255); //$ Red-purple
					else if(kw.focus4) color=Color.lightGray; //% Light gray
					else if(kw.focus5) color=new Color(128,128,255); //& Blue-purple
					else if(kw.focus6) color=new Color(0,128,0); //? Dark green
				...
				if(kw.set||kw.mouseset) g.drawRect(x-sw+1,y-sh+1,sw*2-2,sh*2-2);
				if(kw.link1seed){
					g.setColor(Color.green);
					g.drawRect(x-sw-5, y-sh-5, sw*2+10, sh*2+10);
				}
				if(kw.weakkw && !kw.weaksearchflg){
					if(kw.strongany) g.setColor(Color.pink); // 2 path
					if(kw.weakall) g.setColor(Color.green); // 1 path
					g.drawRect(x-sw-5,y-sh-5,sw*2+10,sh*2+10);
				}
	...
	private final class Data{
		...
		private int setKeyword(String lbl){
			...
			if(lbl.endsWith("!") || lbl.equals("dummyi") || lbl.equals("dummyj")) kw.dummy=true;
		...
		private void setLij(){
			...
			for(int i=0;i<s;i++){
				...
				if(revR && spring[i].rij!=0) spring[i].lij=1.2*m*spring[i].rij+20;
				else spring[i].lij=m*(1.3-spring[i].rij)+30;
			}
		}
	}
