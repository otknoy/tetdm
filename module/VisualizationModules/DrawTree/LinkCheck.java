...
public class LinkCheck {
  ...
  public static int linkcheck(int init, int link[][], int n) {
    check = new int[n];
    for (int i=0; i<n; i++) {
      check[i] = 0;
    }
    ...
    return check_line(init, link, n);
  }

  public static int check_line(int k, int link[][], int n) {
    int sub_total = 1;

    for (int i=0; i<n; i++)
      if (link[k][i] == 1 && check[i] == 0) {
        check[i] = 1;
        sub_total += check_line(i,link,n);
      }
