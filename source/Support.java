...
//		if(class_num == 2)g2.setColor(Color.blue);
  ...
  void set_image1(Graphics2D g2,int i) {
    ...
    if (data.task[i].canopen == true) {
      data.task[i].image = door[1];  //can open
    }
    if (i == support.click_task) {
      data.task[i].image = door[3];  // click or touch now
    } else if (data.task[i].click_num % 2 == 1) {
      data.task[i].image = door[3];  //click(open) now
    }
    if (data.task[i].clear == true) {
      data.task[i].image = door[2];  //clear
    }
