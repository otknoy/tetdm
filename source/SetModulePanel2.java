...
//		upperlayer.add(toolinfolabel);	//TEMPORARY NOT USE
	...
	public void set_noderelate()
	{
		...
		for(i=0;i<map2.node_num;i++){
			for(j=0;j<map.node_num;j++)visual_mining[i][j]=-1;					
		}
	...
	public void compare_pairnum()
	{
		...
		for(int i=0;i<map.node_num;i++)cchair[i]=-1;
		for(int i=0;i<map2.node_num;i++)vchair[i]=-1;
		...
		for(int i=0;i<map2.node_num;i++){
			...
			if(visual_pairnum[i][1]!=0)visual_haspair+=1;
    ...
    {		
		...
		if(changeY > 1)changeY *= 1.2;
		letter_size = (int)((double)letter_size * changeY);
		imageSize = (int)((double)imageSize * changeY);

		if(letter_size <= 10)letter_size = 10;
		if(letter_size > maxLetterSize)letter_size = maxLetterSize;
...
//                                        (int)((panelX[j]+panelX[j+1])/2), panelY[0] + panelSizeY/3 + panelLetterSize/4*3);
	...
	public void drawMiningArrow(Graphics2D g2, int n)
	{
		...
		if(map.node[miningIndex[n]].focusClickExecute == true || map.node[miningIndex[n]].focusTouchExecute == true)
        {
			...
			if(map.node[miningIndex[n]].focusClickExecute == true && map.node[miningIndex[n]].focusTouchExecute == true)
            {
				CCC=1.0/5.0;	TTT=4.0/5.0;
			}
            else
            {
				CCC=1.0/2.0;	TTT=1.0/2.0;
			}
...
//		System.out.println("Call repaint");
		...
		public void mouseDragged(MouseEvent me)	//Dragging
		{
			if(moving == true)		//Move a node
			{
				try{
					...
					if(mousex>sizeX-60)mousex = sizeX-60;
					if(mousey<upperlayer.getSize().height+5)mousey = upperlayer.getSize().height+5;
					if(mousey>sizeY)mousey = sizeY-5;
					if(touchNumber != -1){
	...
	public int checkNames(int k,int num) {
		...
		for(int i=97;i<123;i++){
			...
			if(k==0)count += countMatches(map.node[num].name,test);
			if(k==1)count += countMatches(map2.node[num].name,test);
		}
		for(int i=40;i<42;i++){
			test=String.valueOf((char)i);
			if(k==0)count += countMatches(map.node[num].name,test);
	...
	public int checkInformation(int k,int num) {
		...
		for(int i=97;i<123;i++){
			test=String.valueOf((char)i);
			if(k==0)count += countMatches(control.moduleData.miningInformationText[num],test);
			if(k==1)count += countMatches(control.moduleData.visualizationInformationText[num],test);
		}
		for(int i=40;i<42;i++){
			test=String.valueOf((char)i);
			if(k==0)count += countMatches(control.moduleData.miningInformationText[num],test);
...
//					map2.node[i].name_length = map2.node[i].name.length()+1;
					...
					for(int j=0;j<setToolPanel.moduleInformation.visuModuleChecked[i].focusVariables2Exist.length;j++){
						if(setToolPanel.moduleInformation.visuModuleChecked[i].focusVariables2Exist[j]==true)
                        {
							...
							if(j==0)map2.node[i].touch4501=true;
							if(j==1)map2.node[i].click4502=true;
						}
					}
					for(int j=0;j<setToolPanel.moduleInformation.visuModuleChecked[i].focusFunctionsExist.length;j++){
						if(setToolPanel.moduleInformation.visuModuleChecked[i].focusFunctionsExist[j]==true){
							...
							if(j==0)map2.node[i].repaintOthersByTouch=true;
							if(j==1)map2.node[i].repaintOthersByClick=true;
						}
					}
					for(int j=0;j<setToolPanel.moduleInformation.visuModuleChecked[i].focusFunctions2Exist.length;j++){
						if(setToolPanel.moduleInformation.visuModuleChecked[i].focusFunctions2Exist[j]==true){
							...
							if(j==0||j==2)map2.node[i].executeByTouch=true;
							if(j==1||j==3)map2.node[i].executeByClick=true;
						}
					}
					...
					if(map2.node[i].focusTouchExecute == true || map2.node[i].executeByTouch == true)map2.node[i].focus_num += 1;
					if(map2.node[i].focusClickExecute == true || map2.node[i].executeByClick == true)map2.node[i].focus_num += 1;
					if(map2.node[i].repaintOthersByTouch == true)map2.node[i].focus_num += 1;
					if(map2.node[i].repaintOthersByClick == true)map2.node[i].focus_num += 1;
